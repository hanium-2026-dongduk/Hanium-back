...
| `npm run smoke:week5` (`week5.smoke.js`) | 콘텐츠 전체 흐름 | 인증·프로필 → 동화 읽기·즐겨찾기 → 단어 미션·단어장 → 퀴즈 조회·채점 → 포인트·대시보드 |
...
DB_HOST=127.0.0.1 DB_PORT=3306 DB_NAME=hanium_smoke DB_USER=root DB_PASSWORD= npm run smoke:week5
...
Week5 스모크는 Gemini·이미지·TTS 외부 API를 호출하지 않는다. 외부 API 비용과 일시 장애로
CI가 흔들리지 않도록 생성 완료 상태의 동화와 퀴즈 fixture를 실제 MySQL에 넣은 뒤, 이후의
모든 HTTP·서비스·DB 흐름을 검증한다. AI 생성 품질과 외부 연동은 운영 전 수동 점검 대상이다.
