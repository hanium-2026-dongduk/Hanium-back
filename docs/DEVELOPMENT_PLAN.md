...
## Week 2 — 자녀/보호자 + 동화 생성 핵심

| 날짜 | 개발자 A | 개발자 B |
|------|----------|----------|
| **Mon** | `child_profiles` 모델 + 생성 API | `characters` 모델 + 캐릭터 생성/조회 API |
| **Tue** | 자녀 프로필 조회/수정/삭제 API | `story_characters`, `story_settings` 모델 정의 |
| **Wed** | `guardian_settings` 모델 + PIN 설정/검증 API | 동화 생성 API 구현 (키워드+캐릭터+설정 → Gemini) |
| **Thu** | 일일 사용 제한 시간 설정/조회 API | 동화 생성 API 완성 (pages + illustrations + tts 트랜잭션 저장) |
| **Fri** | 사용 시간 차감/체크 미들웨어 | `story_page_illustrations`, `story_page_tts` 모델 + 동화 상세 조회 API |

### 산출물
- A: 자녀 CRUD + 보호자 PIN + 시간 제한 미들웨어
...
| **Thu** | `reward_wallets` 모델 + 포인트 적립 서비스 | `quiz_sets`, `quiz_questions`, `quiz_options` 모델 정의 |
| **Fri** | 레벨업 로직 + streak_days 갱신 | 퀴즈 자동 생성 API (동화 기반 → Gemini → 구조화 저장) |

### 산출물
- A: 출석 + 미션 + 포인트 적립 + 레벨업 시스템
- B: 즐겨찾기 + 단어장 + 퀴즈 자동 생성
...
| **Wed** | `praise_stickers`, `sticker_sends` 모델 + 스티커 발송/조회 API | 퀴즈 결과 조회 API + 오답 노트 |
...
```
Week 1 Fri ─── users 모델 + auth 미들웨어 확정
                 │
Week 2 Mon ─── child_profiles 확정 ──→ B의 모든 테이블 FK 참조 가능
                 │
Week 3 Thu ─── rewardService 인터페이스 확정 ──→ B가 Week 4에 연동
                 │
